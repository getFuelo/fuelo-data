...
const families = ['ap41','ap71','ap36','ap7-cartagena-vera','ap66','r2','ag55','ag57','r3','r5','r4','ap9-frontera','ap9-centro','ap9-sur','ap9-ferrol','ap9-norte','ap61','ap68','ap1-closed','ap8-bizkaia','ap8-gipuzkoa-west','ap8-gipuzkoa-east','supersur','ap53','ap636','ap9-combined','ap8-ap1-shared','iberpistas'];
...
for (const [family, file] of [['ap53','terminals'],['ag55','port'],['r2','full'],['ap68','public-approaches']]) {
...
const report = {catalogOrdersChecked: 2, complete: false, total: cases.length, passed: 0, failures: [], byFamily: {}};
for (const c of cases) {
 ...
 const matches = result => result.quote.status === 'quoted' && result.quote.minCents === c.expected && result.quote.maxCents === c.expected;
 ...
 if (ok) {report.passed++; counts.passed++;} else {
  counts.failed++;
...
console.log(JSON.stringify({total: report.total, passed: report.passed, failed: report.failures.length, byFamily: report.byFamily}, null, 2));

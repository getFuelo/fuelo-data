...
ROOT=Path(__file__).resolve().parents[2];parser=argparse.ArgumentParser();parser.add_argument('family',choices=['ap41','ap71','ap36','ap7-cartagena-vera','ap66','r2','ag55','ag57','r3','r5','r4','ap9-frontera','ap9-centro','ap9-sur','ap9-ferrol','ap9-norte','ap51','ap6','ap61','ap68','ap1-closed','ap8-gipuzkoa-west','ap8-bizkaia','ap8-gipuzkoa-east','supersur']);parser.add_argument('--limit',type=int);args=parser.parse_args();family=args.family
...
for fare in net['pricing']['fares']:
 ...
 if journey and journey.get('via'):
  body['locations'][1:1]=[{'lat':p['lat'],'lon':p['lng'],'type':'through'} for p in journey['via']]
 raw=urllib.request.urlopen('https://valhalla1.openstreetmap.de/route?json='+urllib.parse.quote(json.dumps(body)),timeout=45).read();d=json.loads(raw);assert d.get('trip'),d;target.write_bytes(raw);(out/(name+'-request.json')).write_text(json.dumps({'request':body,'expectedGeneralCents':fare['tariff']['baseCents'],**provenance.get('probeAnnotations',{}).get(name,{})},indent=2)+'\n');print(name,d['trip']['summary']['length'],d['trip']['summary']['has_toll'],flush=True);fetched+=1;time.sleep(.6)

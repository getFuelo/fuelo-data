...
def extract(id,downloads,source):
 ...
 return {'kind':'tariff-reference','complete':False,'checked':'2026-09-16','currency':'EUR','vehicle_class':'light','validFrom':'2026-01-01','validThrough':('2026-11-10' if id=='es-ap68' else '2026-12-31'),**({'validitySource':'https://www.boe.es/boe/dias/2026/09/11/pdfs/BOE-A-2026-19039.pdf','validityNote':'Concession ends 2026-11-10; no tariff assumption after that date. This decree concerns heavy-vehicle discounts, not new light-vehicle amounts.'} if id=='es-ap68' else {}),'timeZone':'Europe/Madrid','source':source,'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'ods':rows,'notes':['General light-vehicle rows only; physical accesses and route evidence still required.','Published names preserved, including source typos, pending explicit alias mapping.','Do not add joint AP-6 journey rows to a separately quoted AP-6 journey.','Unlisted pairs are unknown; free sections require separate evidence.']}
def main():
